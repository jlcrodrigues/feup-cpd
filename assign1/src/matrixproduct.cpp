	...
	for (int i = 0; i < m_ar; i += bkSize) {
        for (int j = 0; j < m_br; j += bkSize) {
            for (int k = 0; k < m_ar; k += bkSize) {
                ...
                for (int a = i; a < min(i + bkSize, m_ar); a++) {
                    for (int b = k; b < min(k + bkSize, m_br); b++) {
                        for (int c = j; c < min(j + bkSize, m_ar); c++) {
...
int main (int argc, char *argv[])
{
	...
	do {
		...
		switch (op){
  		...
  		printf("L1 DCM: %lld \n",values[0]);
  		printf("L2 DCM: %lld \n",values[1]);
		printf("PAPI TOT CYC: %lld \n",values[2]);
  		printf("PAPI TOT INS: %lld \n",values[3]);
		printf("PAPI DP OPS: %lld \n",values[4]);
